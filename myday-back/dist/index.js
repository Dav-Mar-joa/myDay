  ...
  "node_modules/@colors/colors/lib/colors.js"(exports2, module2) {
    ...
    function applyStyle() {
      ...
      while (i--) {
        ...
        if (newLinesPresent) {
          str = str.replace(newLineRegex, function(match3) {
            return code.close + match3 + code.open;
          });
        }
      }
      return str;
      ...
      remove(transport) {
        ...
        if (!isStream(transport) || transport.log.length > 2) {
          target = this.transports.filter(
            (match3) => match3.transport === transport
          )[0];
        }
        if (target) {
          this.unpipe(target);
...
var connectToMongo = async () => {
  try {
    ...
    Logger_default.info("Link to mongodb etablished");
  } catch (error) {
    Logger_default.error("Fail to connection mongodb", error);
    process.exit(1);
  }
};
...
var responseSchema = new import_mongoose2.default.Schema({
...
// src/routes/register.routes.ts
var registerRouter = (0, import_express.Router)();
registerRouter.post("/", async (request, response) => {
  const { pseudo, password, feelings, mail, phraseRegister } = request.body;
  const theme = request.body.theme || "colorful";
  const pseudoExist = await User_default.findOne({ pseudo });
  const mailExist = await User_default.findOne({ mail });
  if (pseudoExist) {
    return response.status(409).json({ success: false, message: "Pseudo d\xE9j\xE0 utilis\xE9" });
  } else if (mailExist) {
    return response.status(409).json({ success: false, message: "Email d\xE9j\xE0 utilis\xE9" });
  } else {
    ...
    try {
      ...
      response.status(201).json({ message: "Utilisateur cr\xE9\xE9 avec succ\xE8s" });
    } catch (err) {
...
var register_routes_default = registerRouter;
...
router.get("/user-feelings", async (req, res) => {
  if (!req.session.user) {
    return res.status(401).json({ message: "Non autoris\xE9 : utilisateur non connect\xE9" });
  }
  try {
    const user = await User_default.findById(req.session.user.id);
    if (!user) {
    ...
    console.error(err);
    res.status(500).json({ message: "Erreur serveur lors de la r\xE9cup\xE9ration des \xE9motions" });
  }
});
router.get("/user-phraseGratitude", async (req, res) => {
  if (!req.session.user) {
    return res.status(401).json({ message: "Non autoris\xE9 : utilisateur non connect\xE9" });
  }
  try {
    ...
    res.status(500).json({ message: "Erreur serveur lors de la r\xE9cup\xE9ration des phrases" });
  }
});
router.get("/api/feelings", async (req, res) => {
  try {
    if (!req.session.user) {
...
router.post("/feelings", async (req, res) => {
  try {
    ...
    await user.save();
    res.status(200).json({ message: "Feeling mis \xE0 jour et historique effac\xE9 avec succ\xE8s" });
  } catch (reason) {
...
router.get("/history", async (req, res) => {
  ...
  try {
    const user = await User_default.findById(req.session.user.id);
    if (!user) {
      return res.status(404).json({ error: "Utilisateur non trouv\xE9" }).end();
    }
    ...
    res.status(500).json({ message: "Erreur serveur lors de la r\xE9cup\xE9ration de l'historique" }).end();
  }
});
...
router2.post("/", async (req, res) => {
  ...
  try {
    const userLogged = await User_default.findOne({ pseudo });
    if (!userLogged) {
      return res.status(400).json({ errorMessage: "Login ou mot de passe erron\xE9 !" });
    }
    ...
    if (userLogged.isAdmin === "y") {
      console.log("Utilisateur admin connect\xE9");
...
router2.get("/check-auth", async (req, res) => {
  if (req.session.user) {
    const pseudo = req.session.user.username;
    try {
      const user = await User_default.findOne({ pseudo });
      if (user) {
...
var login_routes_default = router2;

// src/routes/logout.routes.ts
var import_express4 = require("express");
var router3 = (0, import_express4.Router)();
router3.post("/logout", (request, response) => {
  if (request.session.user) {
    request.session.destroy((err) => {
      if (err) {
        return response.status(500).json({ message: "Erreur lors de la d\xE9connexion" });
      }
      response.clearCookie("connect.sid", { path: "/" });
      response.status(200).json({ message: "D\xE9connexion r\xE9ussie" });
    });
  } else {
    response.status(400).json({ message: "Aucun utilisateur connect\xE9" });
  }
});
var logout_routes_default = router3;

// src/routes/authentification.route.ts
...
var authentification_route_default = router4;

// src/middleware/Authenticate.ts
var authenticated = (request, response, next) => {
...
var Authenticate_default = authenticated;
...
if (process.env.NODE_ENV === "development") {
  ...
  app.use((0, import_cors.default)({
    credentials: true,
    origin: "*"
    // Need to update this Line with good deployment
  }));
}
...
app.use("/api", (req, res, next) => {
  Logger_default.info(`${req.method} - ${req.originalUrl}`);
  next();
});
app.use("/api/login", login_routes_default);
app.use("/api/logout", Authenticate_default, logout_routes_default);
app.use("/api/register", register_routes_default);
app.use("/api/check-auth", authentification_route_default);
app.use("/api/user", Authenticate_default, users_routes_default);
...
connectToMongo().then(() => app.listen(port, () => {
  Logger_default.info(`Server is running on http://localhost:${port}`);
}));
